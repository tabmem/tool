...
#################################################################
# basic parsing of model responses
#################################################################


def parse_feature_string(s, feature_names, as_dict=False, in_list=False, final_delimiter=","):
    ...
    while len(s) > 3:
        ...
        # if there isn't, then the value is between the magic string and the final delimitier, or the of the string
        else:
            end = -1
            if final_delimiter is not None:
                end = s[start + len(magic_strings[feature_index]) :].find(final_delimiter)
            if end > -1:
...
def get_prefix_features(csv_file, p: float):
    ...
    IMPORTANT: this function is guaranteed to return a true prefix, i.e. at most 1 and at least len(feature_names) - 1 features.
    """
