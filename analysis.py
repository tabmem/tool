...
from utils import load_csv_rows, get_delimiter, find_nth, accuracy
import utils
...
def build_first_token(csv_file):
    ...
    """
    csv_rows = load_csv_rows(csv_file, header=False)
    num_rows = len(csv_rows)
    for num_digits in range(1, np.min([len(x) for x in csv_rows])):
